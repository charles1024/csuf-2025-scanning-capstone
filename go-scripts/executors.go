...
// checks if the HTTP response indicates an Apache server
func isApacheServer(scanCtx *scan.ScanContext) bool {
	// try to get the server header from the scan context
	if scanCtx != nil && scanCtx.Input != nil {
		// check if there's response data available
		if scanCtx.Input.MetaInput != nil && scanCtx.Input.MetaInput.CustomMap != nil {
			if serverHeader, ok := scanCtx.Input.MetaInput.CustomMap["server"]; ok {
				if serverStr, ok := serverHeader.(string); ok {
					return strings.Contains(strings.ToLower(serverStr), "apache")
				}
			}
		}
	}
...
func isApacheTemplate(template *templates.Template) bool {
